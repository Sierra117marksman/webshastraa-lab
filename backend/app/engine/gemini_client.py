...
# Track models that recently hit 429 or 503 so multi-step tasks skip exhausted/overloaded models immediately
...
def generate_content_with_retry(
    ...
    """
    Executes LLM content generation with zero-delay multi-model cascade & Groq LPU failover:
    ...
    # 1. Primary: Attempt Google Gemini cascade, skipping models currently in quota/503 cooldown
    for current_model in models_to_try:
        ...
        try:
            ...
            if is_quota:
                cooldown_secs = 600.0 if is_daily_quota else 60.0
                _MODEL_COOLDOWN_UNTIL[current_model] = time.time() + cooldown_secs
                logger.warning(
                    f"[Gemini 429] {current_model} quota reached (cooldown {int(cooldown_secs)}s). Immediately cascading..."
                )
                continue
            ...
            if is_transient:
                ...
                logger.warning(
                    f"[Gemini 503] {current_model} experiencing high demand (cooldown 120s). Immediately cascading..."
                )
                continue

            logger.warning(f"[Gemini Error] {current_model} failed ({err_msg[:90]}). Immediately cascading...")
            continue

    # 2. Fallback: If Gemini cascade failed or exhausted quota, switch to Groq LPU
    logger.warning(f"[LLM Engine] Gemini cascade exhausted ({last_error}). Switching to Groq LPU...")
